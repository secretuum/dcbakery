...
6 файлов в `supabase/migrations/20260731000X_*` (dialog_state, processed_messages, consents,
voice_messages + приватный бакет, lead_drafts, **registration_tokens** — хэши одноразовых
рег-токенов). Прогнать по порядку в Supabase SQL Editor (или `supabase db push`). Идемпотентны
(`CREATE TABLE IF NOT EXISTS`). Ничего существующего не меняют.

## Регистрация по одноразовой ссылке

Новому клиенту после первого заказа бот шлёт ссылку `…/register?rt=<токен>`. В БД хранится
только **хэш** токена (sha256), сам токен — в URL; single-use, TTL 30 мин, привязан к номеру.
Страница `/register` читает токен (peek, без гашения — чтобы префетч не сжёг), показывает
форму с **подтверждённым номером** (только чтение); клиент задаёт пароль/компанию → регистрация
идёт существующим потоком `/api/profile/register` → код в WhatsApp → кабинет. Токен гасится при
успешной отправке (`/api/register-link/consume`). Сессия по клику НЕ выдаётся.
