...
test("хендофф → бот молчит, а через 15 минут авто-возобновляет ответы (Q84)", async () => {
  const t = setup();
  t.setAgent(agentOut({ intent: "handoff", reply: "Передаю менеджеру." }));
  await handleIncomingMessage(msg({ messageId: "r1", text: "позовите человека" }), t.deps);
  assert.equal(t.state(), "human_handoff");

  // Меньше 15 минут: новое сообщение — бот молчит (ничего не отправлено, состояние держится).
  ...
  // Прошло больше 15 минут (тест сдвигает активность на 61 мин назад) → бот снова отвечает.
  t.staleDialog();
