      ...
      >
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            className="-mr-1 -mt-1 text-2xl leading-none text-muted hover:text-dark"
          >
            ×
          </button>
        </div>
        ...
        {isOtp ? (
          <>
            <p className="mt-3 text-sm leading-6 text-muted">
              {t("Мы отправили код на ")}<b className="text-dark">{prefill.phone}</b>{t(". Введите 6 цифр — код действует 2 минуты.")}
            </p>
            <Input
          ...
          <>
            <p className="mt-3 text-sm leading-6 text-muted">{t("Войдите — и заказ оформится сразу.")}</p>
            <label className="mt-4 block">
              <span className="text-sm font-bold text-dark">{t("Почта или номер телефона")}</span>
              <Input className="mt-2" value={login} onChange={(e) => setLogin(e.currentTarget.value)} placeholder="company@example.com" />
            </label>
          ...
          <>
            <p className="mt-3 text-sm leading-6 text-muted">
              {t("Заказы оформляются только с подтверждённым аккаунтом. Придумайте пароль — код подтверждения придёт в WhatsApp на ")}<b className="text-dark">{prefill.phone}</b>.
            </p>
            <div className="mt-4 rounded-btn bg-cream px-4 py-3 text-xs font-semibold text-muted">
            ...
            <label className="mt-3 block">
              <span className="text-sm font-bold text-dark">{t("Пароль для кабинета")}</span>
              <Input
                className="mt-2"
                type="password"
                value={password}
