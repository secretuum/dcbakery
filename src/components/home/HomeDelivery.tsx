 ...
 * Бизнес-правила актуальные: минимальный заказ 15 000 ₸; доставка по Алматы всегда
 * бесплатная; вт · чт · сб, приём до 18:00.
 */
export function HomeDelivery() {
  const t = useT();

  const steps: { n: string; title: string; text: string }[] = [
    { n: "01", title: t("Каталог"), text: t("Соберите корзину в каталоге — минимальный заказ 15 000 ₸.") },
  ...
  const infoCards: { eyebrow: string; value: string; note: string }[] = [
    ...
    },
    {
      ...
      value: t("15 000 ₸"),
      note: t("Заказы дешевле пока не оформляем"),
    },
    {
      eyebrow: t("Стоимость доставки"),
      value: t("Бесплатно"),
      note: t("По всему Алматы, на любой заказ"),
    },
  ];
